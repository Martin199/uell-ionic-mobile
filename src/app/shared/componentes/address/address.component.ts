...
export class AddressComponent {
  ...
  formConfigColombia() {
    this.addressForm.get('street')?.setValidators([Validators.required, Validators.pattern(/^[a-zA-Z0-9.\-#\s]+$/)]);
    this.addressForm.get('street')?.updateValueAndValidity();
    this.addressForm.get('number')?.setValidators([Validators.maxLength(5), Validators.pattern(/^[a-zA-Z0-9\-#\s]+$/)]);
    this.addressForm.get('number')?.updateValueAndValidity();
    this.addressForm.get('floor')?.setValidators([Validators.pattern(/^[a-zA-Z0-9]+$/), Validators.maxLength(3)]);
    this.addressForm.get('floor')?.updateValueAndValidity();
    this.addressForm.get('apartment')?.setValidators([Validators.pattern(/^[a-zA-Z0-9]+$/), Validators.maxLength(3)]);
    this.addressForm.get('apartment')?.updateValueAndValidity();
    this.addressForm
      .get('postalCode')
      ?.setValidators([Validators.pattern(/^[0-9]\d*$/), Validators.maxLength(6), Validators.minLength(6)]);
    ...
    this.addressForm.get('observation')?.setValidators([Validators.maxLength(250)]);
    this.addressForm.get('observation')?.updateValueAndValidity();
  }

  private getProvincias() {
    // this.adressForm.controls.Provincia.disable();
    this.userService.getAddressesState(this.country).subscribe((res: any) => {
      res.forEach((e: any) => {
