...
import { IFilePostBodyImage, IResponsePostLikes, IWellnessPortalPost } from 'src/app/pages/tabs/interfaces/wellness-portal-interfaces';
...
export class DetailsWellnessPortalComponent {
  ...
  async getPostById() {
    ...
    this.wellnessPortalService.getPostById(this.postId)
    .pipe(
      switchMap((resp: IWellnessPortalPost) => {
        ...
        this.filesService.downloadFile(resp.body_image.url).pipe() : of(null);
      })
    )
    .subscribe({
      next: (res: string | null) => {
        if (res) {
          ...
          this.post.title_image.content = file.fileContent ? file.fileContent : '../../../../assets/imgs/Uell_Img.jpg';
        }
        this.loadImage = true;
        loading.dismiss();
  ...
  countLikes(event: any){
    const id = this.activatedRoute.snapshot.paramMap.get('id');
    if (event && id) {
      this.wellnessPortalService.postLikes(parseInt(id), {liked: true, disliked: false}).subscribe( (reactPost: IResponsePostLikes) => {
        if (reactPost) {
          this.wellnessPortalService.readPostlikes(parseInt(id)).subscribe( res => {
  ...
  countDislikes(event: any){
    const id = this.activatedRoute.snapshot.paramMap.get('id');
    if (event && id) {
      this.wellnessPortalService.postLikes(parseInt(id), {disliked: true, liked: false}).subscribe( (reactPost: IResponsePostLikes) => {
        if (reactPost && id) {
          this.wellnessPortalService.readPostlikes(parseInt(id)).subscribe( res => {
            this.dislikes = res.dislikes;
            this.likes = res.likes;
