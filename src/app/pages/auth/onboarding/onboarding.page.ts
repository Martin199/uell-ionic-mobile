...
export class OnboardingPage implements AfterViewInit, OnInit, OnDestroy {
  ...
  async nextSlide() {
    ...
    if (this.step === 8) {
      ...
      if (
        this.tenantParameters.activeModules.find((x: any) => x === 'isps') &&
        this.tenantParameters.excludeISPSFromOnboarding !== 'true'
      ) {
        this.goIsps();
      } else {
  ...
  prevSlide(): void {
    ...
    this.progress !== 0.125 ? (this.progress = this.progress - 0.125) : (this.progress = 0.125);
    if (this.swiperContainer.nativeElement.swiper) {
  ...
  private postMedicalInfo(body: any, bodyTwo: any) {
    const data: MedicalHistoryDiseases = {
      medicalHistoryDiseases: {
